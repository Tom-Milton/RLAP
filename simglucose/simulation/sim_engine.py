...
class SimObj(object):
    ...
    def simulate(self):
        obs = self.env.reset()
        tic = time.time()
        while self.env.env_time < self.env.start_time + self.sim_time:
            if self.animate:
                self.env.render()
            action = self.controller.select_action(obs, evaluate=True)
    ...
    def save_results(self):
        ...
        filename = os.path.join(self.path, str(self.env.patient_name) + '.csv')
        df.to_csv(filename)
...
def batch_sim(sim_instances, parallel=False):
    ...
    if parallel and pathos:
        ...
        results = [sim(s) for s in tqdm(sim_instances)]
    toc = time.time()
